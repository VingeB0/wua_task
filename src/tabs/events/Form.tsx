...
const gradient: string = 'relative before:right-0 before:left-0 before:-top-2 before:content before:absolute before:w-full before:h-2 before:bg-gradient-to-t before:from-grayGradientStart before:to-grayGradientEnd before:opacity-40';
...
export const Form = ({increaseEvents, ...props}: FormProps) => {
    ...
    return (
        <form
            ...
            className={`mt-auto py-5 px-8 ${gradient}`}
            method="POST" action="/">
            <div className="flex items-stretch gap-3 relative">
                <input
                    ...
                    type="text" name="comment" placeholder="Додайте коментар"/>
                <button
                    className={`font-bold text-17 bg-primaryBlue text-white py-2 px-5 rounded-5 ${animation}`}>Додати
                </button>
            </div>
