...
export const TabItem = ({text, onClick, active, tab, ...props}: TabItemProps) => {
    ...
    return (
        <li {...props}>
            <button
                className={`flex items-center h-full py-2 px-7 border-x border-t border-solid border-gray ${active ? `bg-white cursor-auto ${decorLine}` : `bg-gray ${animation}`}`}
                onClick={onClick}
                type="button"
            >
                {text}
                {TABS.EVENT === tab && (
                    <div className={`ml-2 relative ${!active && 'pointer-events-none'}`}>
                        <Popup content={<EventFilter setOption={handleOptionClick} option={selectedOption}/>}>
                            <span className={`flex items-center cursor-pointer hover:underline text-primaryBlue`}>{actionFilterToText[selectedOption]}{isOpen ? <ChervonDown className="transform scale-[-1]"/> : <ChervonDown/>}</span>
                        </Popup>
                    </div>
                )}
