...
const animation: string =
  "transition-opacity duration-300 ease-linear hover:opacity-60";
...
export const TabItem = ({
  ...
  const [selectedOption, setSelectedOption] = useState<ACTION_FILTERS>(
    ACTION_FILTERS.ALL,
  );
      ...
      >
        ...
        {TABS.EVENT === tab && (
          <div className={`ml-2 relative ${!active && "pointer-events-none"}`}>
            ...
            >
              <span
                className={`flex items-center cursor-pointer hover:underline text-primaryBlue`}
              >
                {actionFilterToText[selectedOption]}
                {isOpen ? (
                  <ChervonDown className="transform scale-[-1]" />
                ) : (
                  <ChervonDown />
                )}
              </span>
